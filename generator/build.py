...
S = 3      # картон полок, мм
W_IN = 120            # ширина в свету (между боками)
W = W_IN + 2 * T      # наружная ширина шкафа = 124
D_UP = 50             # глубина витрины (наружная, с задником)
D_LOW = 60            # глубина тумбы (наружная, с задником)
DRAWER_D = D_LOW - 10 # глубина коробки ящика
...
NICHE = 60            # ниша в свету
FLOOR_Y = NICHE       # дно витрины 62..65
SHELF3_Y = FLOOR_Y + S + 38       # 101..104
SHELF2_Y = SHELF3_Y + S + 38      # 142..145
TOPP_Y = SHELF2_Y + S + 52        # 197..199
H_UP_SIDE = TOPP_Y + T            # 199
H_CAP = S
H_TOTAL = H_LOW_SIDE + H_TOP + H_UP_SIDE + H_CAP   # 300
...
assert H_TOTAL == 300, H_TOTAL
HANDLE_CX = (W / 2 - 31, W / 2 + 31)   # центры ручек ящиков
PANEL_W, PANEL_H = DOOR_W - 16, LDOOR_H - 20   # филёнка
MARKS_UP = [FLOOR_Y, FLOOR_Y + S, SHELF3_Y, SHELF3_Y + S, SHELF2_Y, SHELF2_Y + S, TOPP_Y]
MARKS_LOW = [H_PLINTH, H_PLINTH + T, DRAWER_SUP_Y, DRAWER_SUP_Y + S]
def marks_txt(ms, pairs=True):
    c = [cm(m) for m in ms]
    return "; ".join(f"{c[i]}/{c[i+1]}" for i in range(0, len(c) - 1, 2)) + (f"; {c[-1]}" if len(c) % 2 else "") + " см"
...
PARTS = [
    # витрина
    ("В1", "Бок витрины", D_UP - T, H_UP_SIDE, T, 4, "разметить полки: " + marks_txt(MARKS_UP), "side_up"),
    ...
    # тумба общая
    ("Т1", "Бок тумбы", D_LOW - T, H_LOW_SIDE, T, 4, f"разметка: дно {cm(H_PLINTH)}/{cm(H_PLINTH+T)} см; в правом шкафу опора {cm(DRAWER_SUP_Y)}/{cm(DRAWER_SUP_Y+S)} см", "side_low"),
    ...
    ("Л2", "Филёнка на дверцу", PANEL_W, PANEL_H, T, 2, "фигурная накладка, по желанию", "panel"),
    # правый низ
    ("П1", "Опора ящика (правый шкаф)", W_IN, D_LOW - T, S, 1, f"картон 3 мм, верх на {cm(DRAWER_SUP_Y+S)} см от пола", "rect"),
    ...
    ("П4", "Дно ящика", W_IN - 3, DRAWER_D, T, 1, "зазор 0,15 см с каждой стороны", "rect"),
    ("П5", "Бок ящика", DRAWER_D, 20, T, 2, "стоит на дне ящика", "rect"),
...
def door_cabinet_front(svg, x0, right=False):
    ...
    if not right:
        for dx in (0, DOOR_W + DOOR_GAP):
            ...
            px = x0 + dx + (DOOR_W - PANEL_W) / 2
            svg.rect(px, H_PLINTH + (LDOOR_H - PANEL_H) / 2, PANEL_W, PANEL_H, "panel")
        # замочные скважины
        ...
        for y in FRONT_YS:
            ...
            for hx in HANDLE_CX:   # центры ручек
...
def front_view():
    ...
    s.dim_v(-30, 0, DOLL_H, f"кукла {DOLL_H}", ext=-20)
    s.dim_v(TOTAL_W + 18, 0, H_LOW_SIDE + H_TOP, f"тумба {UP_BASE}", ext=TOTAL_W + 4, side="right")
    s.dim_v(TOTAL_W + 18, H_LOW_SIDE + H_TOP, H_TOTAL, f"витрина {H_UP_SIDE + H_CAP}", ext=TOTAL_W + 4, side="right")
    s.dim_v(TOTAL_W + 34, 0, SHELF_TOP_Y, f"верх полки {SHELF_TOP_Y}", ext=TOTAL_W + 20, side="right")
...
def section_view(right=False):
    ...
    if right:
        ...
        s.rect(D_LOW - DRAWER_D, DRAWER_SUP_Y + S, DRAWER_D, T, "cut")   # дно ящика
        s.rect(D_LOW - DRAWER_D, DRAWER_SUP_Y + S + T, T, 20, "cut")  # задняя стенка ящика
    ...
    xl = -12
    s.dim_v(xl, 0, H_PLINTH, ext=0)
    s.dim_v(xl, H_PLINTH + T, H_LOW_SIDE, ext=0)
    s.dim_v(xl, UP_BASE, UP_BASE + FLOOR_Y, f"ниша {NICHE}", ext=0)
    s.dim_v(xl, UP_BASE + FLOOR_Y + S, UP_BASE + SHELF3_Y, ext=0)
    s.dim_v(xl, UP_BASE + SHELF3_Y + S, UP_BASE + SHELF2_Y, ext=0)
    s.dim_v(xl, UP_BASE + SHELF2_Y + S, UP_BASE + TOPP_Y, ext=0)
    xl2 = -40
    s.dim_v(xl2, 0, H_LOW_SIDE, f"бок тумбы {H_LOW_SIDE}", ext=0)
    s.dim_v(xl2, UP_BASE, UP_BASE + H_UP_SIDE, f"бок витрины {H_UP_SIDE}", ext=0)
    s.dim_v(-66, 0, H_TOTAL, ext=0)
    # справа: отметки от низа бока витрины
    xr = D_LOW + 22
    s.dim_v(xr, UP_BASE, UP_BASE + FLOOR_Y, ext=D_LOW + 8, side="right")
    s.dim_v(xr, UP_BASE, UP_BASE + SHELF3_Y, ext=D_LOW + 8, side="right")
    s.dim_v(xr + 14, UP_BASE, UP_BASE + SHELF2_Y, ext=D_LOW + 8, side="right")
    s.dim_v(xr + 28, UP_BASE, UP_BASE + TOPP_Y, ext=D_LOW + 8, side="right")
    if right:
        s.dim_v(xr, 0, DRAWER_SUP_Y, ext=D_LOW + 8, side="right")
    s.dim_v(xr + 14, 0, H_PLINTH, ext=D_LOW + 8, side="right")
    # глубины
    s.dim_h(0, D_LOW + OVER, -10, f"столешница {D_LOW + OVER}", ext=0)
    s.dim_h(0, D_UP + OVER, H_TOTAL + 8, f"карниз {D_UP + OVER}", ext=H_TOTAL)
    s.dim_h(0, D_UP, UP_BASE + NICHE / 2)
    s.dim_h(0, D_LOW, H_LOW_SIDE / 2)
...
def door_drawing():
    ...
    s.dim_v(DOOR_W + 4, 0, FRAME_BOT, ext=DOOR_W, side="right")
    s.dim_v(DOOR_W + 4, FRAME_BOT, FRAME_BOT + WIN_H, ext=DOOR_W, side="right")
    s.dim_v(DOOR_W + 4, FRAME_BOT + WIN_H, VDOOR_H, ext=DOOR_W, side="right")
...
def drawer_drawing():
    ...
    s = Svg(W + 100, 42, pad=(20, 14, 20, 26), scale=2.6)
    # проём
    ...
    s.dim_h(T + 1.5, W - T - 1.5, -7, f"ящик {W_IN - 3}", ext=0)
    s.dim_h(0, W, 38, f"фасад {W}", ext=32)
    s.dim_v(W + 8, S, S + T + 20, "коробка 22", ext=W, side="right")
    s.dim_v(W + 20, 0, FRONT_H, f"фасад {FRONT_H}", ext=W, side="right")
    s.dim_v(W + 32, FRONT_H, 26 + S, "3", ext=W, side="right")
    s.text(W / 2, 12, "до столешницы: фасад 0,3 · коробка 0,4", "lbl")
...
<style>{CSS}</style>
<main>
<div class="eyebrow">Палитурный картон 2 мм · полки 3 мм · размеры в см · под куклу 28 см · компактная версия</div>
...
<p class="lead">Компактная версия: шкафы уже (12,4 см вместо 15,4), мельче (5 и 6 см вместо 6 и 7) и на 1 см ниже, чтобы кукла по центру не терялась. Все размеры уже с учётом толщины картона и склейки «торец к плоскости». Обводить можно прямо по таблице деталей или распечатать PDF-шаблоны 1:1.</p>
<div class="keys">
<div class="key"><b>30</b><span>высота шкафа, см (кукла 28 + 2)</span></div>
<div class="key"><b>12,4</b><span>ширина шкафа, см</span></div>
<div class="key"><b>5 / 6</b><span>глубина витрины / тумбы, см</span></div>
<div class="key"><b>32,2</b><span>ширина композиции по корпусам, см (32,8 по карнизам)</span></div>
<div class="key"><b>12</b><span>внутри между боками, см</span></div>
</div>

<h2>Фасад</h2>
<figure>{fv}<figcaption>Вид спереди. Столешница и карниз выступают на 0,3 см по бокам и спереди, сзади заподлицо с задником. Полка между шкафами (готовая, 2,2×7,4×3 см) клеится к бокам на высоте 16,1 см от пола по верхней грани, вровень с полом витрины.</figcaption></figure>
...
<h2>Карта деталей</h2>
<p>Все размеры в сантиметрах. Количество указано сразу на оба шкафа. Ширина везде первая, высота (или глубина для горизонтальных панелей) вторая. Ориентация «ширина × глубина» у полок: 12 — поперёк шкафа, 4,8 — от фасада к заднику.</p>
{parts_table()}

<h2>Отступы на клей: не нужны</h2>
<div class="note">
<p>Ты клеишь торец одной детали к плоскости другой, поэтому никаких клапанов и припусков не требуется. Вместо припусков в размерах уже учтена толщина картона: например, верхняя панель витрины 12 см, а не 12,4, потому что она встаёт между боками толщиной по 0,2 см.</p>
...
<ol>
<li><b>Разметка боков.</b> На всех боках витрины В1 проведи линии на 6; 6,3; 10,1; 10,4; 14,2; 14,5 и 19,7 см от нижнего края: это низ и верх дна витрины, полок и низ верхней панели. На боках тумбы Т1 отметь 1 и 1,2 см, а на двух боках правого шкафа ещё 6,6 и 6,9 см.</li>
...
<li><b>Ящик.</b> Дно П4, на нём бока П5, между ними стенки П6. Проверь ход в проёме, потом приклей фасад П3 так, чтобы он перекрывал коробку ящика на 0,35 см по бокам, 0,3 см снизу и 0,1 см сверху; между фасадом и столешницей остаётся 0,3 см. Ложные фасады: средний на 3,8–6,4 см от пола, нижний на 1–3,6 см, зазоры 0,2 см. В щели над средним фасадом виден торец опоры П1, его стоит закрасить. Ручки по две на фасад, центры на 3,1 и 9,3 см от левого края (между ручками 2 см, до краёв по 1 см).</li>
<li><b>Стыковка.</b> Витрину ставь на столешницу вровень по заднику и приклей по нижним торцам боков и дна ниши; спереди столешница выступит на 1,3 см и получится полка ниши для лампы и телефона. Между шкафами полка 7,4 см на высоте 16,1 см по верху.</li>
</ol>

<h2>Проверка по декору (см)</h2>
<div class="check">
<div>Верхняя полка витрины в свету <b>5,2</b> · чайник 4,1, кукла 3,4 ✓</div>
<div>Средняя и нижняя полки в свету <b>3,8</b> · стопка книг 2,8, часы 1,8 ✓</div>
<div>Ширина за стеклом <b>12</b> · кукла 2,3 + чайник 4,5 = 6,8 на верхней полке ✓, тарелки 4,5 переезжают на нижнюю полку. Средняя полка: часы + часы + книги + книги = 10,1 ✓ (окна 2×4,7, стойки рамок 0,7 см частично прикрывают крайние предметы)</div>
<div>Ниша под планкой спереди <b>4,8</b>, в глубине <b>6</b> · лампа 4,6 + салфетка 0,1 ✓ (у самой кромки запас 0,1 см, глубже свободно; ниша 12 см: телефон 3 + лампа 3 + салфетка 5,6 ✓)</div>
<div>Глубина полок <b>4,7</b> · тарелки 3, полка между шкафами 3 ✓</div>
<div>Фасад ящика <b>12,4 × 2,6</b> · две ручки 4,2 × 0,9 ✓ (впритык, если тесно — по одной ручке по центру)</div>
<div>Ниша правого шкафа · сундук 2,5 × 3,4 × 2,4 ✓</div>
<div>Задник комнаты · рекомендуемая высота <b>33–35</b> (кукла + 5…7), ширина около <b>36–40</b></div>
</div>
</main>
"""
...
def build_pdf():
    ...
    def draw_part(x, y, part):
        ...
        if shape in ("side_up", "side_low"):
            marks = MARKS_UP if shape == "side_up" else MARKS_LOW
            ...
            c.setDash()
            if shape == "side_low":
                c.setFont(font, 8); c.drawString(x + 12 * MM, y + 72 * MM, f"{cm(DRAWER_SUP_Y)}/{cm(DRAWER_SUP_Y+S)} — ТОЛЬКО 2 бока"); c.drawString(x + 12 * MM, y + 75.5 * MM, "правого шкафа")
