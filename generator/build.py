...
OVER = 3              # свес столешницы и карниза по бокам и спереди
...
VDOOR_H = TOPP_Y - (FLOOR_Y + S) + T   # 142 (от верха дна витрины до верха верхней панели)
FRAME_SIDE, FRAME_TOP, FRAME_BOT = 9, 14, 12
WIN_W = DOOR_W - 2 * FRAME_SIDE   # 58
WIN_H = VDOOR_H - FRAME_TOP - FRAME_BOT  # 116
GLASS_W, GLASS_H = WIN_W + 10, WIN_H + 10

LDOOR_H = H_LOW_SIDE - H_PLINTH   # 85
FRONT_H = 27                      # фасад ящика
FRONT_YS = [H_PLINTH, H_PLINTH + FRONT_H + 2, H_PLINTH + 2 * (FRONT_H + 2)]  # 10, 39, 68
...
PARTS = [
    ...
    ("В8", "Рамка дверцы витрины", DOOR_W, VDOOR_H, T, 4, f"окно {WIN_W}×{WIN_H}, поля 9/14/12", "vdoor"),
    ...
    ("Т5", "Столешница", W + 2 * OVER, D_LOW + 2 * OVER, S, 2, "3 мм, сзади заподлицо, свес 3 мм", "rect"),
    # левый низ
    ...
    ("П2", "Заглушка под ложные фасады", W_IN, DRAWER_SUP_Y - (H_PLINTH + T), T, 1, "вертикально, заподлицо с фасадом", "rect"),
    ...
    ("П5", "Бок ящика", 60, 21, T, 2, "стоит на дне ящика", "rect"),
    ("П6", "Передняя и задняя стенка ящика", W_IN - 3 - 2 * T, 21, T, 2, "между боками ящика", "rect"),
]
...
def door_cabinet_front(svg, x0, right=False):
    ...
    for dx in (x0 + 1, x0 + W - 1):
        for hy in (ub + FLOOR_Y + S + 15, ub + FLOOR_Y + S + VDOOR_H - 25):
...
def section_view(right=False):
    ...
    if right:
        ...
        s.rect(D_LOW - 60, DRAWER_SUP_Y + S + T, T, 21, "cut")        # задняя стенка ящика
        s.rect(D_LOW - T, DRAWER_SUP_Y + S + T, T, 21, "cut")         # передняя стенка ящика
        for y in FRONT_YS:
            s.rect(D_LOW, y, T, FRONT_H, "door")
    else:
        s.rect(D_LOW, H_PLINTH, T, LDOOR_H, "door")
    s.rect(0, H_LOW_SIDE, D_LOW + 2 * OVER, H_TOP, "shelf")          # столешница
    # витрина
    ...
    s.rect(D_UP, UP_BASE + FLOOR_Y + S, T, VDOOR_H, "door")
    ...
    # глубины
    s.dim_h(0, D_LOW + 2 * OVER, -10, "столешница 76", ext=0)
...
def door_drawing():
    ...
    s.dim_h(0, FRAME_SIDE, -8, "9", ext=0); s.dim_h(FRAME_SIDE, FRAME_SIDE + WIN_W, -8, "58", ext=0); s.dim_h(FRAME_SIDE + WIN_W, DOOR_W, -8, "9", ext=0)
    s.dim_v(DOOR_W + 14, 0, VDOOR_H, "142", ext=DOOR_W, side="right")
...
def drawer_drawing():
    ...
    s.rect(T + 1.5, S + T, T, 21, "cut"); s.rect(W - T - 1.5 - T, S + T, T, 21, "cut")
    s.rect(0, S - 1, W, FRONT_H, "doorline")             # фасад (контур)
...
<h2>Фасад</h2>
<figure>{fv}<figcaption>Вид спереди. Столешница и карниз выступают на 3 мм по бокам и спереди. Полка между шкафами (готовая, 22×74×30) клеится к бокам на высоте 163 мм от пола по верхней грани, вровень с полом витрины.</figcaption></figure>
...
<li><b>Планка и дверцы.</b> Планка В7 клеится на передние торцы так, чтобы её верх был вровень с верхом дна витрины (закрывает торец 3 мм и висит в нишу на 12 мм). Дверцы витрины В8 накладные: закрытые лежат на передних торцах боков, верх вровень с верхом верхней панели, низ на планке. Петли снаружи: одно крыло на бок, второе на дверцу, по две на дверцу, на 15 мм от верха и 25 мм от низа. Дверцы тумбы Л1 так же, от цоколя до столешницы.</li>
<li><b>Ящик.</b> Дно П4, на нём бока П5, между ними стенки П6. Проверь ход в проёме, потом приклей фасад П3 так, чтобы он перекрывал коробку ящика на 3,5 мм по бокам, 1 мм снизу и 3 мм сверху. Ложные фасады: средний на 39–66 мм от пола, нижний на 10–37 мм, зазор 2 мм. Ручки по две на фасад, центры на 39 и 115 мм от левого края.</li>
<li><b>Стыковка.</b> Витрину ставь на столешницу вровень по заднику: спереди столешница выступит на 16 мм и получится полка ниши для лампы и телефона. Между шкафами полка 74 мм на высоте 163 мм по верху.</li>
</ol>
...
<div>Ширина за стеклом <b>150</b>, видно через окна <b>116</b> · кукла + чайник + тарелки 113 ✓</div>
<div>Ниша под планкой спереди <b>48</b>, в глубине <b>62</b> · лампа 46 + салфетка 1 ✓</div>
<div>Глубина полок <b>57</b> · тарелки 30, полка между шкафами 30 ✓</div>
<div>Фасад ящика <b>154 × 27</b> · две ручки 42 × 9 ✓</div>
...
def build_pdf():
    ...
    def draw_part(x, y, part):
        ...
        if shape == "valance":
            ...
            n_ = 6; seg = W_ / n_
            for i in range(n_):
                x1 = x + W_ - seg * (i + 1); p.curveTo(x1 + seg * .75, y - 2 * MM, x1 + seg * .25, y - 2 * MM, x1, y + 4 * MM)
